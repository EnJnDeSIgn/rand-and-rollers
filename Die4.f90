program Die4_roller_EnJnDeSIgn2024
    ...
    do j = 1, num_iterations
        ...
        do i = 1, roll_count
            ...
            if (selected_numbers(i) == '0') then
                call random_number(rand)
                final_numbers(i) = int(rand * 2)	! Values 0-1
            else if (selected_numbers(i) == '1') then
                call random_number(rand)
                final_numbers(i) = int(rand * 2)+2	! Values 2-3
            end if
	...
	print *, "Mode 0-3:"
		...
		d4_value = final_numbers(position_index)
		d4 = d4_value

		! Adjust d4 to be in the range 1-4
		d4 = mod(d4, 4) + 1

		print *, "Random D4: ", d4		
end program Die4_roller_EnJnDeSIgn2024